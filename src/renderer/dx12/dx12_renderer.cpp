...
std::filesystem::path cg::renderer::dx12_renderer::get_shader_path(const std::string& shader_name)
{
	...
	auto shader_path = std::filesystem::path(path_buffer).parent_path() / shader_name;
	return shader_path;
}
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	for (size_t i = 0; i < model->get_vertex_buffers().size(); ++i) {
		...
		vertex_buffers[i]->SetName(vertex_buffer_name.c_str());
		...
		index_buffers[i]->SetName(index_buffer_name.c_str());
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	command_list->ResourceBarrier(
			_countof(end_barriers), end_barriers
	);
